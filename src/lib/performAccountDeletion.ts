// Gemeinsame Lösch-Logik für beide Bestätigungs-Pfade (Link und 8-Zeichen-Code).
// Nimmt einen bereits validierten Deletion-Token, führt die kaskadierende
// Löschung durch und versendet die Nachweis-Mail. Idempotent: Wenn der User
// nicht mehr existiert, gibt sie „already_deleted" zurück, ohne zu fehlern.

import { createClient as createAdminClient } from "@supabase/supabase-js";
import nodemailer from "nodemailer";
import { verifyDeletionToken } from "./deletionToken";
...
export type DeletionResult =
  ...
  | { ok: false; reason: "invalid" | "expired" | "config" | "delete_failed" };

export async function performAccountDeletionByToken(token: string): Promise<DeletionResult> {
  if (
  ...
  if (!existing?.user) {
    // Doppelter Klick oder Code doppelt eingelöst – kein Fehler.
    return { ok: true, status: "already_deleted" };
  }

  // Zählen, was gelöscht wird – für die Success-Mail.
  ...
  if (deleteError) {
    console.error("[performAccountDeletion] auth.admin.deleteUser:", deleteError.message);
    return { ok: false, reason: "delete_failed" };
  }

  // Alle Codes dieses Users in der Fallback-Tabelle aufräumen, damit sie
  // nicht ungenutzt herumliegen und die Lookup-Tabelle nicht wächst.
  try {
    await admin.from("account_deletion_codes").delete().ilike("email", email);
  } catch {
    /* Aufräumen ist nicht fatal */
  }
  ...
  if (
    ...
    try {
      ...
      const transactionId = `DEL-${now.getTime().toString(36).toUpperCase()}-${userId
        .slice(0, 8)
        .toUpperCase()}`;
...
<body style="margin:0;padding:0;background-color:#F5F9F9;font-family:Arial,Helvetica,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" role="presentation"
    ...
    <tr>
          ...
          <tr>
            <td style="background-color:#ffffff;padding:28px 22px;
              ...
              <p style="margin:0 0 20px 0;color:#1A1A1A;font-size:15px;line-height:1.6;">
                ...
                abgeschlossen. Nachfolgend finden Sie den Nachweis der
                entfernten Daten.
              </p>
                ...
                <tr>
                  <td style="padding:14px 16px;">
                    ...
                    <p style="margin:0 0 4px 0;color:#1A1A1A;font-size:14px;line-height:1.5;">
                      Zeitpunkt: <strong>${escapeHtml(formattedDate)}</strong>
                    </p>
                    <p style="margin:0 0 4px 0;color:#555555;font-size:13px;line-height:1.5;">
                      Zeitzone: Europe/Berlin
                    </p>
                    <p style="margin:0;color:#555555;font-size:13px;line-height:1.5;">
                      Vorgangs-ID:
                      <span style="font-family:'Courier New',Courier,monospace;">${escapeHtml(transactionId)}</span>
                    </p>
                  </td>
                </tr>
              </table>
      ...
      console.error("[performAccountDeletion] Success email error:", emailErr);
    }
  }
